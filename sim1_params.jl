...
# ╔═╡ da2d0cae-1fb5-47b4-b75c-70c0364f449f
using DifferentialEquations

# ╔═╡ 75a53656-6027-4556-acb0-889613e72b29
using Unitful

# ╔═╡ 31225fe8-dcea-4be5-94a6-1c407e25b078
# Plot solutions in the time domain
using Plots

# ╔═╡ 3f39ed29-e16a-4beb-92c2-808fc00844a6
using PlutoUI

# ╔═╡ 27ab1bb1-71d8-42f0-a785-38e12a31eac5
using LaTeXStrings

# ╔═╡ 2aa51df0-6d54-11f0-1799-5b3d63d320b0
md"""
## FitzHugh-Nagumo (Morning Oscillator) and Goldbeter (Molecular Clock) Oscillator Models
"""

# ╔═╡ 9ea8f22e-cfa9-40a3-85bf-a07a2fb47079
md"""
### Units

In order to specify and scale units/dimensions with Julia, we must specify them explicitly with Unitful.jl. Before creating or using a new unit, check that it is already defined by inspected the Unitful namespace: `isdefined(Unitful, :unit)`
"""

# ╔═╡ 04b4487f-0e5d-4bb3-aa86-254e006b5c2f
@unit μM " μM" μM (10^(-6))*u"mol" true

# ╔═╡ 1531b549-986c-475e-9d5f-123e76899262
@unit ms " ms" ms (10^(-3))*u"s" true

# ╔═╡ 0423fdf0-6c66-4293-a696-7414a376ca5a


# ╔═╡ 1ca8defb-642c-4aac-ab11-ce90442ad4ba
md"""
### (1.a.) FHN Parameter Declaration 
"""

# ╔═╡ adb21d39-259b-49af-8692-9d1b068e5704
N = 1        # mumber of neurons

# ╔═╡ 584d90df-32a6-4b46-a62d-b84c8bcf81b3
a = 0.7      # threshold parameter (affects nullcline shape, controls spiking)

# ╔═╡ 1b650970-be7b-45e5-a6e7-09fc21791eb8
# ╠═╡ disabled = true
#=╠═╡
b = 0.8      # recovery parameter (affects timescale of w, the recovery variable)
  ╠═╡ =#

# ╔═╡ 2531ca3a-897d-45f7-9101-2d44c34c27b0
ε₀ = 0.08    # timescale separation (ε << 1 ⇒ fast-slow dynamics); base value at T_ref

# ╔═╡ be41209c-0c5c-4381-ba65-921e2be0d277
I_ext = 0.5  # input current from outside the cell (constant input to all neurons if N > 1)

# ╔═╡ 1e755e26-8952-4c4e-9655-bc8d72ad0ad0
α = 5.0      # slope of the coupling sigmoid (steepness of interaction)

# ╔═╡ 127e062f-2cf1-4436-8023-a9ca66bd887d
v₀ = a       # Resting potential / midpoint = threshold

# ╔═╡ dd372c2e-050c-4bb8-a2f9-7b670ddd2730
...
# ╔═╡ 71c4aa57-a402-4707-ab20-13834d80684a
# Placeholder for connectome-derived matrix (if added later)
C_E = []  # To be filled with empirical data later

# ╔═╡ 2f4649d9-2587-42f3-96ae-8ef1ef4faf72
# Set connectivity
C = C_T

# ╔═╡ bf7957d3-8f54-4e30-8568-9d6810b1e224
...
# ╔═╡ e1c85d90-3739-4057-a062-239251698da3
# Sigmoid function for nonlinear coupling 
function σ(v, α, v₀)
    1 / (1 + exp(-α * (v - v₀)))
end

# ╔═╡ 49a1cd99-3d36-42ee-87ae-921d9e8b4fbe
# (1.b.) FHN Function  
...
# ╔═╡ ef9c2190-6a06-4f4c-8eac-857721f0ffc2
# (1.c.) Run the FitzHugh-Nagumo model
u0 = [-1,1]                                  # Initial conditions for v and w

# ╔═╡ 4b9150d8-a9e8-4ab0-8935-f08c6685ee50
tspan = (0, 200)                               # Time span for the simulation

# ╔═╡ d851d8ca-c44b-4e53-becb-50a9385bd0b6
#=╠═╡
p = (N, a, b, ε₀, I_ext, α, v₀, C)                 # Parameters for the model
  ╠═╡ =#

# ╔═╡ 1ad56c69-3e25-4d9b-b7e4-fa8d8f888272
#=╠═╡
prob = ODEProblem(fitzhugh_nagumo!, u0, tspan, p)  # Define the ODE problem
  ╠═╡ =#

# ╔═╡ 153f61d4-3aaa-4db8-8313-f14256ba79d3
#=╠═╡
sol = solve(prob, Tsit5(), saveat=0.1)                   # Solve the ODE problem with Tsit5 method
  ╠═╡ =#

# ╔═╡ f44103f2-f52c-43b4-975a-ef5b912cf280
gr()

# ╔═╡ c66fc8f1-994f-417e-aa46-02461e842c80
#=╠═╡
plot(sol, label=["v(t)" "w(t)"], xlabel="Time", ylabel="State")
  ╠═╡ =#

# ╔═╡ 1fc6ef7b-ef7b-497e-89d7-826ed1e66146
# Parameter Sliders
# @bind N Slider(1:10, show_value=true)

# ╔═╡ a9df994c-905f-4e53-962b-78b4367a35f4
# @bind a Slider(0.0:0.05:1.5, show_value=true)

# ╔═╡ 7d9f217b-1a20-4485-951c-7696f05d0ff5
# @bind b Slider(0.0:0.05:1.5, show_value=true)

# ╔═╡ b84f3cea-32b1-4288-beb4-317a71bfb477
# @bind ε₀ Slider(0.01:0.01:0.2, show_value=true)

# ╔═╡ 87d1e85b-ec22-4979-9a2a-a45942122eee
# @bind I_ext Slider(0.0:0.1:2.0, show_value=true)

# ╔═╡ 27e1dbdc-2691-4998-adff-d12b5cf8e5c1
# @bind α Slider(0.0:0.5:10.0, show_value=true)

# ╔═╡ f9726598-a9f7-4b84-ae6c-f55527859966
#=╠═╡
# plotting the oscillations 
volt_vals  = sol[1, :]
  ╠═╡ =#

# ╔═╡ 1754c13c-f0a1-4c50-9f31-631e0aec5c59
#=╠═╡
time_V = sol.t
  ╠═╡ =#

# ╔═╡ a12bddba-c0cd-49b1-a506-1ab5ba3c410c
#=╠═╡
plot(
    time_V, volt_vals,
    label = "V(t)",
    ...
    linewidth = 2,
    legend = :topright,
    title = "FitzHugh-Nagumo Membrane Potential",
    size = (1000, 600))
  ╠═╡ =#

# ╔═╡ 056f90b9-264c-466d-a4f8-3386f6d38322
#=╠═╡
plot(sol, vars=(1,2), title="Phase Portrait", label=["v(t)" "w(t)"], xlabel="v", ylabel="w")
  ╠═╡ =#

# ╔═╡ 2fedfc02-6646-46ef-ba50-3d10be5ab006
# Display gif inline
# gif(anim_voltage, "fhn_v_animation.gif", fps=60, show_msg=false)

# ╔═╡ 6a7949a3-8985-486d-a658-c8f58b865067
md"""
### (2.a.) Goldbeter Parameter Declaration 
All parameters are derived from Goldbeter (1995) in order to replicate his figures.
"""

# ╔═╡ d9b4f77d-09a6-46ce-9c97-e63eb80d75e7
v_s = 0.76        # max rate at which per (mRNA) accumulates in cytosol [1 μM/hr]

# ╔═╡ 7da02313-f2bc-4378-a389-3c6bc4d3f0a3
v_m = 0.65        # max rate at which per (mRNA) is degraded [1 μM/hr]

# ╔═╡ e16eb984-7087-4e7f-9d15-dfc54542fd1b
K_m = 0.50        # Michalis constant for mRNA degredation [unitless]

# ╔═╡ 822862c9-2f43-4672-9823-b02b67f25284
k_s = 0.38        # rate of synthesis of PER protein [1 μM/hr]

# ╔═╡ 3181bc79-02ee-429a-84df-bbb242ace0cc
v_d = 0.95         # max rate at which fully phosphorylated PER is degraded [1 μM/hr]

# ╔═╡ c2f6dc88-a17a-4a22-96fb-a08eb4caa23c
k_1 = 1.90         # rate constant that that describes the transport of phosphorylated PER into the nucleus [1/hr]

# ╔═╡ 7ab0bb1a-8813-4345-8244-1c970ca38ebf
k_2 = 1.30         # rate constant that that describes the transport of nuclear PER into the cytosol [1/hr]

# ╔═╡ 65085997-9ce2-4c5c-a947-3c49f694ce4a
K_I = 1.00         # threshold constant for repression [μM]

# ╔═╡ 83ee0ea6-9642-4ecb-8445-65828735b187
Kd = 0.20          # Michaelis constant for PER degredation

# ╔═╡ 1efed112-d0e8-40f8-9c44-4857cf118ee3
n = 4.00           # Hill coefficient; measures the ultrasensitivity (sigmoidality) of a Goldbeter–Koshland module [unitless]

# ╔═╡ 70616b9b-46b7-4ca2-aaed-8034a86626e4
# Maximum rate constants of kinases and phosphates involved in phosphorylation and desphosphorylation

# ╔═╡ 53c8af04-e28e-4c5a-8d97-2a17989608f5
K1 = 2.00         # [μM]

# ╔═╡ 4d37e58a-65d6-4281-ab61-6ca5e176f91e
K2 = 2.00         # [μM]

# ╔═╡ 51327b16-619d-4ae2-832e-760cab5cc971
K3 = 2.00         # [μM]

# ╔═╡ 8fb7e3b4-63c3-4971-8f85-8476ad6f2b7e
K4 = 2.00         # [μM]

# ╔═╡ 796e8e7e-c04d-47c3-93e9-15b7c423830f
# Maximum Michaelis constants for the kinases and phosphates involved in phosphorylation and desphosphorylation

# ╔═╡ 9badfae5-ac63-40c7-8a12-076e1ed97a6d
V1 = 3.20         # [1 μM/hr]

# ╔═╡ c69c07b1-dbd9-44f2-b809-c951d0a50e50
V2 = 1.58         # [1 μM/hr]

# ╔═╡ 5f5c6037-da76-4cdc-9ca3-a4c0c1014e22
V3 = 5.00         # [1 μM/hr]

# ╔═╡ aa575889-e4cf-4a23-9d4f-80e12e2d11a8
V4 = 2.50         # [1 μM/hr]

# ╔═╡ 357e1c6a-cddb-479c-8e85-0b20f1ecd1b8
function goldbeter_per!(du, u, p, t)
    ...
    du[1] = ((v_s * K_I^n) / (K_I^n + PN^n)) - ((v_m * M) / (K_m + M))       
    du[2] = (k_s * M) - ( (V1 * P0) / (K1 + P0)) + ((V2 * P1) / (K2 + P1))     
    du[3] = du[3] = ( (V1 * P0) / (K1 + P0)) - ( (V2 * P1) / (K2 + P1)) -   
            ( (V3 * P1) / (K3 + P1)) + ((V4 * P2) / (K4 + P2))              
    du[4] = du[4] = ( (V3 * P1) / (K3 + P1)) - ( (V4 * P2) / (K4 + P2)) -    
            (k_1 * P2) + (k_2 * PN) - ( (v_d * P2) / (Kd + P2))
    du[5] = du[5] = (k_1 * P2) - (k_2 * PN)                               
	# dPN/dt
end

# ╔═╡ 2403cebf-b05d-453c-8141-86dda6b96020
# Initial conditions and parameters

# ╔═╡ 7abbc8af-86b0-4dac-b25c-f50693987f94
# @bind v_s Slider(1:10, show_value=true)

# ╔═╡ 6e00d1a0-9da3-43a9-9c64-ec5ae567a64f
# @bind v_m Slider(1:10, show_value=true)

# ╔═╡ 0b296302-0c46-473c-bc52-1de39dcfb4c0
# @bind K_I Slider(1:10, show_value=true)

# ╔═╡ 68505df1-2bf6-4027-a6e6-9d157afad34b
# @bind K_m Slider(1:10, show_value=true)

# ╔═╡ fa24d909-9c5d-4c10-9d8c-961bdda97e55
# @bind k_s Slider(1:10, show_value=true)

# ╔═╡ c41f1edf-ddd1-4c98-b404-2eb06738174d
# @bind K1 Slider(1:10, show_value=true)

# ╔═╡ 6f9e90c5-8b3f-4716-925b-3ba1419ec734
# @bind K2 Slider(1:10, show_value=true)

# ╔═╡ 8fe647b1-994c-4bdd-901f-81c2a1c132a0
# @bind K3 Slider(1:10, show_value=true)

# ╔═╡ 7f7ca733-40cb-4dc2-b09b-49c0bfd29bdc
# @bind K4 Slider(1:10, show_value=true)

# ╔═╡ 6aef2df3-e6bf-42d1-86c8-6ae26d9fe5bb
# @bind V1 Slider(1:10, show_value=true)

# ╔═╡ 8c865f7b-d4af-4d09-897a-7044dd18405d
# @bind V2 Slider(1:10, show_value=true)

# ╔═╡ 38bb4df8-1fe0-4b23-9032-a79b047a14f2
# @bind V3 Slider(1:10, show_value=true)

# ╔═╡ f9ed198e-47ce-4137-bba9-2fc8bb63ed51
# @bind V4 Slider(1:10, show_value=true)

# ╔═╡ 29c566d4-1db3-4772-b8a1-b10ec9c7f114
# @bind Kd Slider(1:10, show_value=true)

# ╔═╡ 76b76c82-d6dd-4060-8daf-3915df737d98
# @bind n Slider(1:10, show_value=true)

# ╔═╡ 47f5eeee-39cb-4069-9d8b-1ae5a3c51539
# @bind v_d Slider(1:10, show_value=true)

# ╔═╡ 5f6ed58b-f22c-4c8d-8c72-91d80bb0df49
# @bind k_1 Slider(1:10, show_value=true)

# ╔═╡ b030a953-ac15-4598-8382-b16560b1e54a
# @bind k_2 Slider(1:10, show_value=true)

# ╔═╡ ecf7d634-bb74-446b-8f46-16e5f1e0fd41
# Initial concentrations [μM]
M = 0.5               # per mRNA in cytosol

# ╔═╡ 3167c16e-3755-4cf1-bd76-8ded5489e9ab
P0 = 0.5              # unphosphorylated PER protein

# ╔═╡ 02ff95c4-1965-4fa0-a73c-cf62d17e0285
P1 = 0.5              # monophosphorylated PER protein

# ╔═╡ b932cc3a-093f-464a-b39b-b9922bf0c336
P2 = 0.6            # diphosphorylated PER protein

# ╔═╡ 44a5f065-73b4-44d4-8dd8-e95b65ee53e5
PN = 1.5              # nuclear PER protein

# ╔═╡ da0f9d67-0a4c-4e66-9f11-32ee6767ff35
u0_G = [M, P0, P1, P2, PN]  # initial conditions

# ╔═╡ 4bb5b979-06ea-4fad-935b-1f2bdf7b5986
tspan_G = (0.0, 72)     # time in hours

# ╔═╡ e5647adc-eeea-42a0-98dd-1f26eab477b5
...
# ╔═╡ a5b685c5-62d5-41d1-a940-2b8438a79d51
begin
	# Problem definition and solve
	prob_G = ODEProblem(goldbeter_per!, u0_G, tspan_G, p_G)
end

# ╔═╡ e8716522-5bf8-4b92-92f4-02cdbecb575a
sol_G = solve(prob_G, Euler(), dt=0.001)

# ╔═╡ 3211b7ee-a8a0-430b-ab3d-5aa44bfc19a6
# Extract time and variable values
time_G = sol_G.t

# ╔═╡ ae903129-aace-49c3-bf7e-d65abaa89845
M_vals  = sol_G[1, :]

# ╔═╡ 61afd309-12d6-45d1-9d7d-e7a3306480aa
P0_vals = sol_G[2, :]

# ╔═╡ d96965f5-d9df-411f-ac01-58516516dce4
P1_vals = sol_G[3, :]

# ╔═╡ f7e79f8c-ec47-42ba-af23-efa6fb86824c
P2_vals = sol_G[4, :]

# ╔═╡ daa3e489-bc9d-4194-bfb3-53a358505e9d
PN_vals = sol_G[5, :]

# ╔═╡ 788d70c6-1913-4f23-92ca-ed116bae23c3
P_t_vals = P0_vals .+ P1_vals .+ P2_vals .+ PN_vals

# ╔═╡ f75f2126-03d8-4655-a15e-269d98c75d4e
begin
	using CairoMakie
	
	# Define the system — here's a dummy function for illustrative purposes.
	# Replace with real Goldbeter dM/dt and dP_t/dt equations.
	function goldbeter_dynamics(x)
	    M, P = x
	    dM = 1.0 * (1 / (1 + P^4)) - 0.2 * M / (0.5 + M)
	    dP = 0.9 * M - 0.1 * P
	    return Point2f(dM, dP)
	end
	
	# Phase space bounds
	xrange = 0.0 .. 3.0  # M(t)
	yrange = 0.0 .. 3.0  # P_t(t)
	
	# Create streamplot
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel = "M(t) (per mRNA)", ylabel = "Pₜ(t) (Total PER Protein)",
	          title = "Phase Plane of Goldbeter Model")
	
	streamplot!(ax, goldbeter_dynamics, xrange, yrange, colormap = :magma)
	
	# Optional: overlay trajectory
	lines!(ax, M_vals, P_t_vals, color = :orange, linewidth = 2)
	scatter!(ax, [M_vals[1]], [P_t_vals[1]], color = :red, markersize = 10)  # Initial point
	
	fig
	
end

# ╔═╡ 46c5af19-b0e3-49f0-8100-64b832cdc7fe
# Add total M to the left y-axis
p1 = plot(
    time_G, M_vals,
    label = "per mRNA",
    xlabel = "Time (hrs) ",
    ...
    linewidth = 3,
    legend = :topright,
    title = "Circadian oscillations in the Concentration of Cytosolic per mRNA in Drosophila",
    size = (1330, 700))

# ╔═╡ fbd29d7d-de87-404f-9e90-f9936384d2b7
p2 =  plot(
    time_G, P_t_vals,
    label = "Pₜ",
    xlabel = "Time (hrs) ",
	...
	linestyle=:dot,
    legend = :topright,
    title = "Circadian oscillations in the Concentration of PER Protein in Drosophila",
    size = (1000, 600))
	

# ╔═╡ 16612f50-1279-4618-8eb8-c0318857f76d
md"""
Below is a plot of the respective concentrations of `per mRNA` and total PER protein. As expected, peaks in per mRNA precede peaks in per mRNA by several hours.
"""

# ╔═╡ 87aa2339-ebcc-4a01-8742-d782d71bd449
plot(p1, p2, layout=(2, 1))

# ╔═╡ 66b51fc0-cc74-45e9-8eb9-06f1c28dc010
plot(sol_G, label=["M(t)" "P0(t)" "P1(t)" "P2(t)" "PN(t)"], xlabel="Time", ylabel="Concentration [μM]")

# ╔═╡ 57e6d891-b00d-456e-b41f-2f6692915f2d
begin
	p3 = plot(time_G, P0_vals, 
		label = L"P_{0}(t): (conc. unphosphorylated \ PER)",
		xlabel = L"Time \ (hrs)",
		ylabel = L"Concentration \ (μM)",
		left_margin = 5Plots.mm,  # used Plots.jl mm not Unitful.jl
		bottom_margin = 5Plots.mm,
		color = :darksalmon, linestyle = :solid)
	plot!(p3, time_G, P1_vals, 
		  label = L"P_{1}(t): (conc. monophosphorylated \ PER)",
		  color = :blue)
	plot!(p3, time_G, P2_vals,
		  label = L"P_{2}(t): (conc. biphosphorylated \ PER)",
		  color = :aqua)
	plot!(p3, time_G, PN_vals,
		  label = L"P_{N}(t): (conc. nuclear \ PER)",
		  color = :green)
	plot!(p3, time_G, P_t_vals, label = L"P_{tot}(t): \ (conc. total \ PER)",
		  color = :orange, linestyle=:dash)
	plot!(p3, time_G, M_vals,
		  label = L"M(t): (conc. per \ mRNA)",color = :red,
		  title = L"Circadian \ oscillations \ in \ \textit{per} \ mRNA \ and \ PER \ Protein \ Forms \ in \ \textit{Drosophila} \ [μM]",
		  size = (1000, 500))
	savefig("figures/goldbeter_PER.png")  # saves figure, overwriting each time this cell is run
end

# ╔═╡ bc52c5a2-f803-431b-ae08-d8b27fc8712e
#display the figure 
p3

# ╔═╡ 19ef46ea-ae60-45d9-9c11-1ca95c587fe7
md"""
### Limit Cycles
"""

# ╔═╡ c0c95b75-5fa2-433a-91ff-35f90f97a18c
begin
    # Plot limit cycle of Goldbeter model with orange line
    p4 = plot(M_vals, P_t_vals,
        lw = 2, 
        color = :orange,
		xlabel=L" M(t) \ (per \ mRNA)",
		ylabel=L"P_t(t) \ (Total \ PER \ Protein)",
	    title=L"Limit \ Cycle \ of \ Goldbeter \ Model",
		legend=false)

    # Add red marker for initial condition
    scatter!([M_vals[1]], [P_t_vals[1]], 
        marker = :circle, 
        color = :red, 
        markersize = 6,
        label = "Initial Condition")

    savefig("figures/goldbeter_limitCycle.png")
end


# ╔═╡ e5347b85-e879-425e-b068-602e7ff1b303
p4

# ╔═╡ 245b422b-4139-4107-b73c-b8aa19ec5fd6
md"""
### Vector Field Plot (Streamplot) of the Goldbeter Model using CairoMakie.jl

1. First, we define the Goldbeter system as a `struct` or function, plotting mRNA (M) and total PER protein (Pₜ).

2. Then, we define a function f(x) that returns the derivatives: `dM/dt` and `dPₜ/dt`.

3. We can use `streamplot(...)` to draw the vector field over a relevant 2-dimensional Euclidean space.

4. We will be careful to overly the the limit cycle trajectory, if available.

"""

# ╔═╡ 1ad5ceeb-dbb6-4faf-8230-ef7b289ab7dd


# ╔═╡ 23988d8d-3235-4276-83a8-4bdaf3320f35


# ╔═╡ e3b00efd-2245-4e85-918a-82c9e3a2dfea
...
manifest_format = "2.0"
project_hash = "e85923aff9b2a1a08779387d6d47c094676799ac"
...
git-tree-sha1 = "85ecc1ad13ebb4e0fd489bfd1a249cffb3937a51"
